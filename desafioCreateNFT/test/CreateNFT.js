...
describe("CreateNFT", function () {
  it('has initial message', async  () => {
    ...
    await CreateNFT.safeMint(deployer.address, 1);
    // Chequeos
    ...
    expect (await CreateNFT.balanceOf(deployer.address)).to.equal(1); //Que el balance del deployer sea 1
    ...
    expect (await CreateNFT.balanceOf(deployer.address)).to.equal(0); //Que el balance del deployer sea 0
  });
});